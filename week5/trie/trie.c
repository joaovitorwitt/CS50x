...
bool check(char *word)
{
    ...
    int n = strlen(word);
    // setting a node pointer, cursor to the root of the trie
    node *cursor = root;

    // iterate through every letter in word, to determine the array index that corresponds to that letter
    for (int i = 0; i < n; i++)
    {
        // A and a should correspond to 0
        int index = tolower(word[i]) - 'a';

        // if children[index] is a node, reset cursor to this node and check for next letter in its children nodes
        if (cursor -> children[index] != NULL)
        {
            cursor = cursor -> children[index];
